...
jest.mock('@/lib/db/redis', () => ({
  cacheHelpers: { invalidate: jest.fn(), getOrSet: jest.fn() },
}));
...
const cases: Array<[string, string, Handler]> = [
  ['POST', 'http://localhost/api/channels', POST],
  ['DELETE', 'http://localhost/api/channels?type=slack', DELETE],
];

describe('/api/channels CSRF enforcement', () => {
  beforeAll(() => {
